...
2. **Gas smell? Stop here** — see the gas protocol below. Do not continue with
   normal triage.
3. **Landscaping, pool, or pest control? Check the Rentvine "Included Services"
   field FIRST** (see below) — if it's marked **"Owner Responsibility,"** it's
   **not** a tenant work order: route it, don't dispatch or triage.
4. **Check escalation factors** (see below) — they can raise the tier.
5. **Assign the tier** using the definitions below.
6. **Apply the SLA** for that tier.
7. **Output**: the assigned tier, a one-line justification citing the rule that
   drove it, and a recommended next action (who to dispatch / what to do now).
8. **If key facts are missing, STOP and ask** — never assign a tier on a guess.

## Gas smell — fixed protocol (no troubleshooting, no exceptions)

When a tenant reports a gas smell or suspected gas leak, the only response is:
...
- **AC failure in Arizona heat** — no cooling is Emergency (not just Urgent)
  when: outdoor temp is forecast above 100°F within 24 hours, indoor temp has
  reached 95°F+, or a vulnerable tenant (elderly, infant, medical condition) is
  in the unit during any significant heat. Arizona is a hot-climate jurisdiction
  where AC is an essential service — treat it accordingly, not just during
  declared advisories.
...
- **Recurring / repeat issue** — the same problem reported again: bump the
  priority up a tier. If the same system has been reported **3 or more times in
  90 days**, stop dispatching and escalate to the PM — the surface fixes aren't
  holding and a human needs to look at the pattern.

## Stop and escalate immediately — do not handle alone

These situations leave your authority the moment they appear. Stop, pass to the
PM with full context, and do not respond further on your own:

- **Legal language** — if a tenant uses any of these words: "lawyer,"
  "attorney," "habitability," "code violation," "health department," or "housing
  authority" — escalate immediately. The word itself is the trigger; do not wait
  for an actual filing.
- **ADA or accessibility request** — any maintenance request tied to a
  disability or accommodation need. This is a legal determination, not a
  dispatch decision.
- **Health symptom attributed to a condition** — if a tenant connects a symptom
  to a maintenance issue ("headaches from the smell," "breathing trouble from
  the mold") escalate. Do not evaluate the health claim.
- **Building-wide pattern** — multiple units in the same building reporting the
  same issue at once points at a systemic cause. Surface the pattern to the PM;
  do not run individual dispatches.
- **Vendor no-show on Urgent or Emergency** — if a vendor cancels or fails to
  show on a job with a real clock, escalate rather than quietly re-queuing.
- **Contained situation with a path** — once a leak is shut off, a unit is
  secured, and a vendor is scheduled, the situation is contained. Do not keep
  escalating "still pending, still on track." Escalate changes, not non-changes.
...
**Never name the trade to the tenant.** Say "someone will come take a look,"
not "I'm sending a plumber." If you name a trade and a different one shows up,
you've broken a promise. Keep trade specificity for vendor-facing messages only.

**Never tell a tenant "this is your cost"** or assign responsibility on your
own authority. Get the work done; let the PM handle any billing conversation.

### Read for the actual problem, not the keyword

The words a tenant uses are a starting point, not a diagnosis. Route on what
the problem actually is:

- "There's a hole in my wall next to the outlet" → **drywall/handyman**, not
  electrical. The outlet works; the wall is damaged.
- "My breaker keeps tripping" → could be one failing appliance, not a wiring
  fault. Identify the cause before picking a trade.
- A reported gas smell → gas protocol above, not a dispatch.

Routing on the keyword instead of the actual problem is the most common way to
send the wrong vendor. When the trade is genuinely unclear, ask one clarifying
question or request a photo before dispatching.
...
| Outlet **single one-time spark, no smell, no scorch** | Note it; suggest trying a different outlet; dispatch if it recurs |
...
- **Never split a job to stay under the threshold** — breaking one $800 repair
  into two $400 work orders to avoid owner approval is a policy violation, not
  a workaround.
...
- **Never park an emergency waiting on a photo** — ask for photos in parallel
  while the dispatch goes out. Gas protocol is the only exception.
- **Never name the trade to a tenant** — "someone will come take a look," never
  "I'm sending a plumber/electrician/HVAC tech."
- **Never tell a tenant their cost on your own authority** — fix it, document
  it, let the PM handle the billing conversation.
- **Never respond further once legal language appears** — "lawyer," "attorney,"
  "habitability," "code violation," "health department" → stop and hand to PM.
...
Expected: **Emergency.** AC out in mild weather would normally be Urgent, but
the vulnerable-tenant exception (elderly) bumps any heat/AC/water issue up.
Dispatch HVAC immediately; resolve within 24h. Note the escalation reason in
the justification.
...
Expected: **Stop and escalate to PM immediately.** The words "health department"
and "lawyer" are the trigger — do not respond further on your own authority.
Pass to the PM with full context.

### 10. Keyword vs. actual problem
> "Tenant says there's a hole in the wall right next to the electrical outlet."

Expected: **Handyman / drywall, not electrician.** The outlet itself isn't
reported as faulty. Route to handyman for drywall repair. Only if the tenant
reports the outlet is sparking, warm, or scorched does this flip to an
electrical dispatch.
