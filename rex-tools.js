...
const REX_TOOLS = [
  ...
  { name: 'rv_get_lease_balance', description: 'Get outstanding/unpaid charges and total balance due for a lease. Use this when a tenant asks what they owe or when past-due charges need to be explained.', input_schema: { type: 'object', properties: { lease_id: { type: 'string' } }, required: ['lease_id'] } },
  ...
  try {
    switch(toolName) {
      ...
      case 'rv_get_lease_balance': {
        const url = new URL(`${RENTVINE_BASE}/accounting/transactions`);
        url.searchParams.set('leaseID', input.lease_id);
        ...
        const charges = txns.map(t => t.transaction || t).filter(t => t.transactionTypeID === '1' || t.transactionTypeID === 1);
        const unpaid = charges.filter(c => {
          const paid = parseFloat(c.amountPaid || '0');
          const amount = parseFloat(c.amount || '0');
          return paid < amount && c.isVoided !== '1';
        });
        const totalDue = unpaid.reduce((sum, c) => sum + (parseFloat(c.amount || '0') - parseFloat(c.amountPaid || '0')), 0);
        return {
          leaseID: input.lease_id,
          totalDue: totalDue.toFixed(2),
          unpaidCharges: unpaid.map(c => ({ description: c.description, amount: c.amount, amountPaid: c.amountPaid, balanceRemaining: (parseFloat(c.amount||'0') - parseFloat(c.amountPaid||'0')).toFixed(2) }))
        };
      }
      case 'send_sms': {
