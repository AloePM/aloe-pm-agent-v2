        ...
        const extractRes = await anthropic.messages.create({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: 'text', text: `This is a property management invoice or receipt. Extract the following and respond with JSON only:
{
  "propertyAddress": "the ship-to/delivery address (not the bill-to/company address)",
  "appliances": [
    {
      "type": "dishwasher|refrigerator|stove|washer|dryer|microwave|water_heater|ac_unit",
      "brand": "brand name",
      "model": "model number",
      "color": "color if mentioned",
      "cost": "item price as number string e.g. 519.95",
...
Only include appliances, not installation fees or accessories. If a field is not found, use null.` }
            ]
          }]
        });
        ...
        try {
          ...
          invoiceData = JSON.parse(rawText.replace(/```json|```/g, '').trim());
        } catch(e) {
          await client.chat.update({ channel: event.channel, ts: thinking.ts, text: '⚠️ Could not read the invoice. Please make sure it is a clear PDF or image.' });
          return;
        }

        if (!invoiceData.propertyAddress || !invoiceData.appliances?.length) {
          await client.chat.update({ channel: event.channel, ts: thinking.ts, text: `⚠️ I could not find a property address or appliance data in this invoice. Please check the document and try again.` });
          return;
        }

        // Build fields array
        const FIELD_NAME_MAP = {
          dishwasher: { brand: 'Dishwasher brand', model: 'Dishwasher model number', color: 'Dishwasher color', cost: 'Dishwasher purchase cost', datePurchased: 'Dishwasher date purchased' },
          refrigerator: { brand: 'Refrigerator brand', model: 'Refrigerator model number', color: 'Refrigerator color', cost: 'Refrigerator purchase cost', datePurchased: 'Refrigerator date purchased' },
          stove: { brand: 'Range/stove brand', model: 'Range/stove model number', color: 'Range/stove color', cost: 'Range/stove purchase cost', datePurchased: 'Range/stove date purchased' },
          washer: { brand: 'Washing machine brand', model: 'Washing machine model number', cost: 'Washing machine purchase cost', datePurchased: 'Washing machine date of purchase' },
          dryer: { brand: 'Dryer brand', model: 'Dryer model number', cost: 'Dryer purchase cost', datePurchased: 'Dryer date of purchase' },
          microwave: { brand: 'Microwave brand', model: 'Microwave model number', color: 'Microwave color', cost: 'Microwave purchase cost', datePurchased: 'Microwave date purchased' },
          water_heater: { brand: 'Water heater brand', model: 'Water heater model number', cost: 'Water heater purchase cost', datePurchased: 'Water heater date purchased' },
          ac_unit: { brand: 'AC unit 1 brand', model: 'AC unit 1 model number', cost: 'AC unit 1 purchase cost', datePurchased: 'AC unit 1 date purchased' },
        };

        const fields = [];
        for (const appliance of invoiceData.appliances) {
          const map = FIELD_NAME_MAP[appliance.type] || {};
          if (appliance.brand && map.brand) fields.push({ name: map.brand, value: appliance.brand });
          if (appliance.model && map.model) fields.push({ name: map.model, value: appliance.model });
          if (appliance.color && map.color) fields.push({ name: map.color, value: appliance.color });
          if (appliance.cost && map.cost) fields.push({ name: map.cost, value: appliance.cost });
          if (appliance.datePurchased && map.datePurchased) fields.push({ name: map.datePurchased, value: appliance.datePurchased });
        }

        if (!fields.length) {
          await client.chat.update({ channel: event.channel, ts: thinking.ts, text: '⚠️ Found the invoice but could not extract any appliance fields.' });
          return;
        }

        // Call Hub to queue for approval
        ...
        const hubRes = await fetch(`${HUB_URL}/api/agent/update-property-fields`, {
          ...
          body: JSON.stringify({
            ...
            agent: 'ari',
            fields,
            source: `Invoice ${invoiceData.invoiceNumber || ''} from ${invoiceData.vendor || 'vendor'}`,
          }),
        });
        ...
        if (hubData.ok) {
          await client.chat.update({ channel: event.channel, ts: thinking.ts,
            text: `📋 Invoice read — *${invoiceData.propertyAddress}*
${fields.map(f => `• ${f.name}: ${f.value}`).join('\n')}\n\nApproval sent to #maintenance-ari 👆`
          });
        } else {
          await client.chat.update({ channel: event.channel, ts: thinking.ts,
            text: `⚠️ Read invoice but could not queue for Rentvine: ${hubData.error || 'unknown error'}`
          });
        }
        return;
      }
...
Triage this work order: classify urgency (Emergency/Urgent/Routine), identify the Aptly issue type, look up the property's actual city (do not assume Chandler or any default city — confirm it from Rentvine or the property record), suggest the right vendor for that specific city/zone, and flag any escalation triggers. Be concise — this is a Slack notification for the team.`;
    ...
    try {
      ...
      if (issueMatch) {
        // Strip markdown formatting (bold/italic asterisks, backticks) so the exact dropdown string reaches Aptly cleanly
        issueType = issueMatch[1].trim().replace(/[\*_`]+/g, '').trim();
...
slackApp.action('field_update_approve_ari', async ({ ack, body, action }) => {
  await ack();
  ...
  try {
    ...
    console.error('field_update_approve error:', e.message);
  }
});
slackApp.action('field_update_skip_ari', async ({ ack, body, action }) => {
  await ack();
  const pendingId = action.value;
  const HUB_URL = process.env.HUB_URL || 'https://hub.aloepm.com';
  const HUB_TOKEN = process.env.HUB_INTERNAL_SECRET || '';
  try {
    await fetch(`${HUB_URL}/api/agent/field-update-action`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-hub-token': HUB_TOKEN },
      body: JSON.stringify({ pendingId, action: 'skip', responseUrl: body.response_url }),
    });
    await slackApp.client.chat.update({
      channel: body.channel.id,
      ts: body.message.ts,
    ...
    console.error('field_update_skip error:', e.message);
  }
});
