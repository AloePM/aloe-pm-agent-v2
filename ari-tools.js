    ...
    switch(toolName) {
      ...
      case 'aptly_add_comment': {
        const r = await fetch(`https://core-api.getaptly.com/api/board/workOrder/${input.card_id}/comment`, {
          ...
          body: JSON.stringify({ content: `[Automated] ${input.content}`, userId: APTLY_COMMENT_USER_ID })
        });
      ...
      case 'send_sms': {
        ...
        if (input.card_id) {
          ...
          await fetch(`https://core-api.getaptly.com/api/board/workOrder/${input.card_id}/comment`, {
            method: 'POST', headers: { 'x-token': APTLY_TOKEN, 'Content-Type': 'application/json' }, body: JSON.stringify({ content: comment, userId: APTLY_COMMENT_USER_ID })
          });
        }
      ...
      case 'get_vendor_for_trade': {
        ...
        const tradeAliases = {
          ...
          'general repairs': 'general maintenance',
        };
        const rawTradeLower = (input.trade || '').toLowerCase();
        const tradeLower = tradeAliases[rawTradeLower] || rawTradeLower;
        const zoneLower = (input.zone || '').toLowerCase();
        // Find matching trade — match against label or id (schema uses "label", not "name")
        const trade = trades.find(t => {
          const label = (t.label || '').toLowerCase();
          const id = (t.id || '').toLowerCase();
          return (label && (label.includes(tradeLower) || tradeLower.includes(label))) ||
                 (id && (id.includes(tradeLower) || tradeLower.includes(id)));
        });
        if (!trade) return { error: `No trade found for: ${input.trade}`, availableTrades: trades.map(t=>t.label).filter(Boolean) };
        ...
        // Rank exact-zone matches above valley-wide-only matches; priority breaks ties within each group
        const isExactZoneMatch = v => {
          if (!zoneLower) return false;
          const zones = (v.zones||[]).map(z=>z.trim().toLowerCase());
          return zones.includes(zoneLower);
        };
        matched = matched.slice().sort((a,b) => {
          const aExact = isExactZoneMatch(a) ? 0 : 1;
          const bExact = isExactZoneMatch(b) ? 0 : 1;
          if (aExact !== bExact) return aExact - bExact;
          return (a.priority ?? 999) - (b.priority ?? 999);
        });
        return { trade: trade.label, zone: input.zone || 'any', vendors: matched.slice(0,3).map(v => ({ name: v.name, phone: v.phone, zones: v.zones, notes: v.notes, priority: v.priority })), instruction: 'Vendors are sorted with exact-zone matches first, then by priority (lowest number first) within each group. Always dispatch the first vendor listed unless its notes state a specific reason not to (e.g. unavailable, wrong scope, requires pre-approval).' };
      }
      ...
      case 'rv_close_work_order': {
        if (!input.confirmed) {
          return { needsConfirmation: true, message: `This will close Rentvine work order ${input.rv_wo_id} as Completed directly in Rentvine. Only do this if the Aptly card stage was already updated and Rentvine did not auto-sync. Ask staff to confirm before proceeding.` };
        }
        const r = await fetch(`${RENTVINE_BASE}/maintenance/work-orders/${input.rv_wo_id}`, {
          method: 'POST',
          headers: { 'Authorization': `Basic ${RENTVINE_AUTH}`, 'X-Rentvine-Account': process.env.RENTVINE_ACCOUNT, 'Content-Type': 'application/json' },
