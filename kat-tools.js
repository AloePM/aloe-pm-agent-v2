...
async function executeKatTool(toolName, input) {
  try {
    switch(toolName) {
      case 'aptly_get_aptlet_ids': {
        const match = await aptlySearchUnitBoard(input.house_number);
      ...
      case 'rv_search_property': {
        ...
        const res = await rvPropertyLookup(shortQ);
        if (res?.properties?.length) return res;
        // Fallback: try house number only
        const houseNum = (input.address || '').match(/^\d+/)?.[0];
        if (houseNum) {
          const res2 = await rvPropertyLookup(houseNum);
          if (res2?.properties?.length) return res2;
        }
  ...
  return { leaseID: lease.leaseID, startDate: lease.startDate, endDate: lease.endDate, moveInDate: lease.moveInDate, status: lease.primaryLeaseStatusID, tenants };
      ...
      case 'aptly_search_property': {
        const match = await aptlySearchUnitBoard(input.address);
        if (match) return match;
        // Try house number only
        const houseNum = (input.address || '').match(/^\d+/)?.[0];
        if (houseNum) {
          const match2 = await aptlySearchUnitBoard(houseNum);
      ...
      case 'aptly_add_comment': {
        const r = await fetch(`https://core-api.getaptly.com/api/board/8bazEHshdZNuMKCFE/${input.card_id}/comment`, { method: 'POST', headers: { 'x-token': APTLY_TOKEN, 'Content-Type': 'application/json' }, body: JSON.stringify({ content: `[Automated] ${input.content}`, userId: APTLY_COMMENT_USER_ID }) });
